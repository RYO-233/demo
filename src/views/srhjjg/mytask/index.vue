<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="问题类型/类别">
        <treeselect
          v-model="problemId"
          :normalizer="normalizer"
          :options="problemTypeOptions"
          @input="handelChangeProblemType"
          @select="handelSelectProblemType"
          :show-count="true"
          size="small"
          style="width: 200px"
          placeholder="请选择"
    ...
    >
      ...
      <el-tab-pane label="待分配" name="1"  v-if="!checkRole(['handler'])||checkRole(['admin'])">
        <el-badge
          v-if="caseNumObj.待分配 > 0"
          slot="label"
          :value="caseNumObj.待分配"
          class="item"
        >
          <span>待分配</span>
        </el-badge>
      ...
      <el-tab-pane label="待处理" name="2">
        <!--  <el-badge slot="label" :value="caseNumObj.待处理" class="item"  >
          <span>待处理</span>
        </el-badge> -->
      </el-tab-pane>
      <el-tab-pane label="待结案" name="3">
        <el-badge
          slot="label"
          v-if="caseNumObj.待结案 > 0"
          :value="caseNumObj.待结案"
          class="item"
        >
          <span>待结案</span>
        </el-badge>
      </el-tab-pane>
      <el-tab-pane label="已结案" name="4">
        <!-- <el-badge slot="label" :value="caseNumObj.已结案" class="item"  >
          <span>已结案</span>
        </el-badge> -->
      </el-tab-pane>
    </el-tabs>
    ...
    >
      <el-form :model="transferForm" label-position="top" class="transferForm">
        ...
        <el-form-item label="" v-if="transferForm.redirectReasonImg">
          <blImageList
            :imgType="transferForm.caseType === 'grid' ? 'grid' : ''"
            :imgList="transferForm.redirectReasonImg"
          />
        </el-form-item>
...
export default {
  ...
  created() {
     console.log("是否处理人juese");
    const isHandler=this.checkRole(['handler']);
    if(isHandler){
      this.tabModel='2';
  ...
  methods: {
    ...
    /*   this.loading = true; */
      ...
      taskList(this.queryParams).then((res) => {
        if(res.data){
           this.caseNumObj = res.data
        }
        this.srcaseList = res.rows || [];
    ...
    handelChangeProblemType(val) {
      console.log(val);
      if (val) {
        if(this.selectProblem.level==0){
           this.queryParams.problemRootId = val;//大类
          this.queryParams.problemParentId=null;
        }
        if(this.selectProblem.level==1){
           this.queryParams.problemRootId = null;
          this.queryParams.problemParentId=val;//小类
        }
      } else {
        this.selectProblem = {};
        this.queryParams.problemRootId = null;
