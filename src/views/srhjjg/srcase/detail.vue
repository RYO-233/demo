<template>
  <div class="detail-container">
    <div class="page-head">
      ...
      <div class="charges">
        <el-button
          ...
          @click="gotoPage"
          v-if="caseStatus != 1 && caseStatus != 4"
          type="primary"
          size="small"
          >修改</el-button
        >
        ...
        <div class="block" v-if="caseStatus == 0">
          ...
          >
            ...
            <el-form-item label="责任单位" prop="organizationId">
              <responsibility-unit
                class="myinput"
                :inputVal="dataObj.organizationId"
    ...
    <!-- 不立案 -->
    <el-dialog title="不立案" :visible.sync="dialogUnRegister" width="480px"  :close-on-click-modal="false">
      <el-form :model="dataObj">
        <el-form-item label="不立案原因" required="">
          <el-input
            v-model="unhandleReason"
            type="textarea"
    ...
    <!-- 驳回 -->
    <el-dialog title="驳回" :visible.sync="dialogReject" width="480px"  :close-on-click-modal="false">
      <el-form :model="dataObj">
    ...
    <!-- 结案 -->
    <el-dialog title="结案评价" :visible.sync="dialogComment" width="480px"  :close-on-click-modal="false">
      <el-form :model="dataObj">
...
import { getSrcase, updateSrcase, bohuiSrcase } from "@/api/srhjjg/srcase";
  ...
  methods: {
    ...
    getDetail() {
      ...
      getSrcase(this.id).then((res) => {
        ...
        _this.dataObj = data || {};
        //商铺
        if (res.data && res.data.store&&res.data.store.latitude) {
          this.markers[0] = {
    ...
    handleChangeOrganizationId(val) {
      this.dataObj.organizationId = val;
    },
    ...
    noRigisterConfirm() {
      if (!this.unhandleReason) {
        this.$message.error("请输入不立案原因");
      } else {
      const param={
        unhandleReason:this.unhandleReason,
        caseStatus :1,
        id:this.dataObj.id
      }
         updateSrcase(param).then((res) => {
        if (res.code == 200) {
