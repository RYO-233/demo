    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        <div class="head-container">
          <el-input v-model="deptName" placeholder="请输入责任单位名称" clearable size="small" prefix-icon="el-icon-search"
      ...
      <el-col :span="20" :xs="24">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
           <el-form-item label="志愿者姓名" prop="realName" v-if="isVolunteer">
            <el-input v-model="queryParams.realName" placeholder="请输入志愿者姓名" clearable class="formInput"
              @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item label="用户昵称" prop="nickName" v-else-if="isWxGroup">
            <el-input v-model="queryParams.nickName" placeholder="请输入昵称" clearable class="formInput"
              @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item label="用户名称" prop="userName" v-else>
            <el-input v-model="queryParams.userName" placeholder="请输入用户名" clearable class="formInput"
              @keyup.enter.native="handleQuery" />
          </el-form-item>
         
          <el-form-item label="手机号码" prop="phonenumber">
            <el-input v-model="queryParams.phonenumber" placeholder="请输入手机号码" clearable class="formInput"
              @keyup.enter.native="handleQuery" />
          </el-form-item>
          <el-form-item label="状态" prop="status">
            <el-select v-model="queryParams.status" placeholder="用户状态" clearable class="formInput">
          ...
          <el-form-item>
            ...
            <el-button icon="el-icon-refresh" plain type="primary" size="mini" @click="resetQuery">重置</el-button>
          </el-form-item>
        </el-form>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
              v-hasPermi="['system:user:add']" v-if="!isWxGroup">新增</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
            <el-button
              type="success"
        ...
        <bl-table :listData="userList" :queryParams="queryParams" :loading="loading" @getList="getList"
          ...
          /> -->
            <template v-if="isVolunteer">
              <el-table-column label="街道" align="left" key="parentName" prop="dept.parentName"
                :show-overflow-tooltip="true" />
              <el-table-column label="社区（村）" align="left" key="deptName11" prop="dept.deptName"
                :show-overflow-tooltip="true" />
              <el-table-column label="志愿者单位" align="left" key="deptName1" prop="zyOrgName"
                :show-overflow-tooltip="true" />
              <el-table-column label="志愿者姓名" align="center" key="realName1" prop="realName"
                :show-overflow-tooltip="true" />
            </template>
            <el-table-column label="用户昵称" align="center" key="nickName" prop="nickName" v-if="isWxGroup"
              :show-overflow-tooltip="true" />
            <el-table-column label="用户名" align="center" key="userName" prop="userName" v-else-if="!isVolunteer"
              :show-overflow-tooltip="true" />

            <el-table-column label="姓名" align="center" key="realName" prop="realName"
              v-if="columns[2].visible && !isVolunteer" :show-overflow-tooltip="true" />
            <el-table-column label="责任单位" align="center" key="deptName" prop="dept.deptName"
              v-if="columns[3].visible && !isWxGroup && !isVolunteer" :show-overflow-tooltip="true" />
            ...
            <el-table-column label="用户总积分" align="center" key="totalScore" prop="totalScore" v-if="isWxGroup" />
...
<!--             <el-table-column label="注册时间" align="left" key="createTime" prop="createTime" v-if="isWxGroup"
            ...
            <el-table-column label="最近登录时间" align="center" key="loginDate" >
              <template slot-scope="scope">
              ...
              <template slot-scope="scope" v-if="scope.row.userId !== 1">
                ...
                <el-button size="mini" type="text" @click="handleUpdate(scope.row)" v-hasPermi="['system:user:edit']"
                  v-show="!(
                    ...
                    (scope.row.dept && (scope.row.dept.deptId === wxCommonUserDeptId))
                  )
                    ">修改</el-button>
                <el-button v-if="isVolunteer" size="mini" type="text" @click="handleDelete(scope.row)"
                  v-hasPermi="['system:user:remove']"
                  :disabled="(scope.row.dept && (scope.row.dept.deptId === outerSysDeptId))">移除志愿者</el-button>
                <el-button v-else size="mini" type="text" @click="handleDelete(scope.row)"
                  v-hasPermi="['system:user:remove']"
                  :disabled="(scope.row.dept && (scope.row.dept.deptId === outerSysDeptId))">删除</el-button>

                <el-button size="mini" type="text" v-show="scope.row.openId && scope.row.isWxCommonUser !== 1"
                  @click="unbind(scope.row)">微信解绑</el-button>
                <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)"
                  v-hasPermi="['system:user:resetPwd', 'system:user:edit']" v-show="!(
                    (scope.row.dept && (scope.row.dept.deptId === outerSysDeptId)) ||
                    (scope.row.dept && (scope.row.dept.deptId === wxCommonUserDeptId))
                  )
                    ">
    ...
    <!-- 添加或修改用户配置对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="472px" append-to-body :close-on-click-modal=false
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" label-position="top">
        <el-form-item label="用户名" prop="userName" v-if="!isVolunteer">
          <el-input v-model="form.userName" placeholder="请输入用户名(仅支持数字与字母)" maxlength="30" />
        </el-form-item>
        <el-form-item label="志愿者姓名" prop="realName" v-if="isVolunteer">
          <el-input v-model="form.realName" placeholder="请输入志愿者姓名(仅支持数字与字母)" maxlength="30" />
        </el-form-item>
        ...
        <div v-show="isVolunteer">
          <el-form-item label="所属社区" prop="deptId">
            <treeselect v-model="form.deptId" :disable-branch-nodes="true" :normalizer="orgNormalizer"
              :options="volunteerDeptOptions" :show-count="true" placeholder="请选择" />
          </el-form-item>
          <el-form-item label="志愿者单位" prop="zyOrgName">
            <el-input v-model="form.zyOrgName" placeholder="请输入志愿者单位" maxlength="30" />
          </el-form-item>
        </div>
...
export default {
  ...
  data() {
    return {
      checkPhoneNumber: (rule, value, callback) => {
        if (value) {
          const phoneValidateObj = this.rules.phonenumber[this.rules.phonenumber.length - 1];
            ...
            checkPhone({ phonenumber: value }).then(res => {
              if (res.code == 200) {
                if (res.msg == "该手机号可以使用") {
                  ...
                  if (!res.data) {
                    callback(new Error(res.msg + ",继续点击确定可以把对应用户转为志愿者账号"));
                  } else {
                    callback(new Error(res.msg));
                  }
                }
      ...
      rules: {
        userName: [
          ...
          { validator: validateForm.onlyNumberAndEnglish, message: "用户名只支持数字与字母", trigger: "blur" },
          {
            min: 2,
            max: 20,
        ...
        phonenumber: [
          { validator: validateForm.checkPhoneAllowEmpty, message: "请输入正确的手机号码", trigger: "blur" },
        ],
    ...
    isVolunteer(newVal, oldVal) {
      if (newVal) {
        if(this.rules.phonenumber.length === 2) {
          ...
          this.rules.phonenumber.push({ required: true, message: "请输入手机号", trigger: "blur" });
          this.rules.phonenumber.push({ validator: this.checkPhoneNumber2, trigger: 'blur' });
        }
        ...
        this.rules.zyOrgName.push({ required: true, message: "请输入", trigger: "blur" });
      } else {
        if(this.rules.phonenumber.length === 3) {
          this.rules.phonenumber.pop();
          this.rules.phonenumber.pop();
          this.rules.phonenumber.push({ validator: this.checkPhoneNumber, trigger: 'blur' });
        }
  ...
  created() {
    this.rules.phonenumber.push({ validator: this.checkPhoneNumber, trigger: 'blur', dbPhonenumber: null });
    this.getList();
    this.getDeptTree();
  ...
  methods: {
    ...
    getList() {
      ...
      const params={
        ...this.queryParams,
        params:{
          loginDateArr:this.dateRange.length>0?[this.dateRange[0]+" 00:00:00",this.dateRange[1]+" 59:59:59"]:null
        }
      }
      listUserPost(params).then(
        (response) => {
    ...
    handleNodeClick(data, node) {
      ...
      if (data.label == "责任单位用户组" || node.parent.data.label == "责任单位用户组") {
        this.isOrgDept = true;
      }
      if (data.label == "公众用户组") {
        this.isWxGroup = true;
        ...
        this.$refs["form"].validate((valid, fields) => {
          ...
          if (valid || flag) {
            if (this.form.userId != undefined && valid) {
              ...
              if (flag) {
                checkPhone({ phonenumber: this.form.phonenumber }).then(res => {
                  let that = this;
                  if (res.msg !== '该手机号可以使用' && !res.data) {
                    this.$modal.confirm('该手机号已经存在,是否需要转为志愿者账号?').then(function () {
    ...
    handleImport() {
      ...
      if (this.isVolunteer) {
        ...
        this.upload.url = process.env.VUE_APP_BASE_API + "/system/user/importDataZY";
        this.upload.updateSupportInfo = '是否更新已经存在的用户数据(按手机号更新)';
      } else {
        this.upload.title = "用户导入";
        this.upload.url = process.env.VUE_APP_BASE_API + "/system/user/importData";
