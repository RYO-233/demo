...
(function() {
	...
	d.prototype._getRectanglePoint = function(h, g) {
		return [new BMap.Point(h.lng, h.lat), new BMap.Point(g.lng, h.lat), new BMap.Point(g.lng, g.lat),
			new BMap.Point(h.lng, g.lat)
		]
	};
	...
	e.prototype.initialize = function(i) {
		...
		j.style.cssText = "position:absolute;background:url(about:blank);cursor:crosshair;width:" + g.width +
	...
	e.prototype.getDrawPoint = function(l) {
		...
		while (j && j != k.getContainer()) {
			if (!(j.clientWidth == 0 && j.clientHeight == 0 && j.offsetParent && j.offsetParent.nodeName ==
					"TD")) {
				h += j.offsetLeft || 0;
				m += j.offsetTop || 0
			}
	...
	function a(h, g) {
		...
		this.defaultDrawingModes = [BMAP_DRAWING_MARKER, BMAP_DRAWING_CIRCLE, BMAP_DRAWING_POLYLINE,
	...
	a.prototype._generalHtml = function(m) {
		...
		var n = function(o, i) {
			return '<a class="' + o + '" drawingType="' + i + '" href="javascript:void(0)" title="' + h[i] +
				'" onfocus="this.blur()"></a>'
		};
		var k = [];
	...
	a.prototype._setScale = function(j) {
		...
		this.container.style.cssText = ["-moz-transform: scale(" + j + ");", "-o-transform: scale(" + j + ");",
			"-webkit-transform: scale(" + j + ");", "transform: scale(" + j + ");", "margin-left:" + k +
			"px;", "margin-top:" + h + "px;", "*margin-left:0px;", "*margin-top:0px;",
			"margin-left:0px\\0;", "margin-top:0px\\0;",
			"filter: progid:DXImageTransform.Microsoft.Matrix(", "M11=" + j + ",", "M12=0,", "M21=0,",
			"M22=" + j + ",", "SizingMethod='auto expand');"
		].join("")
	};
